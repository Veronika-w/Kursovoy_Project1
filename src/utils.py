...
def read_excel_file(excel_path: str) -> list[dict]:
    ...
    logger.info("Начало работы функции считывающей excel-файл")
    try:
        df_excel = pd.read_excel(excel_path)
        df_dict_excel = df_excel.to_dict(orient="records")
        logger.info("Создан список словарей финансовых транзакций")
...
def filter_by_date(date: str, my_list: list) -> list:
    ...
    list_by_date = []
    logger.info("Начало работы функции фильтрующей данные дате")
    if date == "":
        return list_by_date
    ...
    logger.info("Конец работы функции фильтрующей данные по дате")
    return list_by_date
...
def operations_card(my_list: list) -> Any:
    ...
    logger.info("Начало работы функции с данными по карте")
    ...
    for transaction in my_list:
        if transaction["Номер карты"] == "nan" or type(transaction["Номер карты"]) is float:
            ...
            if transaction["Номер карты"][1:] in cards:
                ...
                cards[transaction["Номер карты"][1:]] = float(str(transaction["Сумма платежа"])[1:])
    logger.info("Формирование списка словарей с данными по картам")
    for k, v in cards.items():
        information_on_cards.append({"last_digits": k, "total_spent": round(v, 2), "cashback": round(v / 100, 2)})
    logger.info("Конец работы функции с данными по карте")
...
def top_five_transactions(my_list: list) -> list[dict]:
    ...
    logger.info("Начало работы функции выводящей топ-5 транзакций")
    for transaction in my_list:
    ...
    logger.info("Формирование списка словарей топ-5 транзакций")
    ...
    logger.info("Конец работы функции выводящей топ-5 транзакцийе")
...
def get_user_settings(path: str) -> list:
    ...
    logger.info("Начало работы функции преобразующей JSON-файл")
    try:
        with open(path, encoding="utf-8") as file:
            try:
                user_settings = json.load(file)
                logger.info("Создан список словарей с данными об валютах и акциях")
...
def currency_rate(currency: dict) -> list[dict]:
    ...
    try:
        ...
        currency_rates = []
        logger.info("Начало работы функции с информацией о курсе валют")
        response = requests.get(url, headers=headers, data={})
        for key, value in response.json().get("rates").items():
            currency_rates.append({"currency": key, "rates": round(1 / value, 2)})
            logger.info("Создание списка словарей с данными о курсе валют")
        logger.info("Конец работы функции с информацией о курсе валют")
...
def currency_stocks(stocks: dict) -> list[dict]:
    ...
    logger.info("Начало работы функции с информацией об акциях")
    ...
    for ind in stocks:
        ...
        logger.info("Формирование списка словарей с данными об акциях")
        stock_prices.append({"stock": ind, "price": quote["c"]})
    logger.info("Конец работы функции с информацией об акциях")
    return stock_prices
