    ...
    try:
        print('Программа: Удалось бы сохранить '
              f'{investment_bank(month_answer,
                 read_excel_file('data/operations.xlsx').to_dict('records'), int(limit_answer))}')
    except ValueError:
...
def reports_option() -> None:
    ...
    if end_date_answer != '':
        ...
        try:
            print(spending_by_category(read_excel_file('data/operations.xlsx'), category_answer, end_date_answer))
        except KeyError:
            print('Программа: возможно вы написали дату не в том формате.')
    else:
        print(spending_by_category(read_excel_file('data/operations.xlsx'), category_answer))
